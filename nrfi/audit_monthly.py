...
def slice_metrics(sf: SnowflakeLoader) -> list[str]:
    ...
    """, [])
    for r in rows:
        lines.append(f"- tier={r['tier']} lineup_confirmed={r['lineup_confirmed']} "
...
def shap_section(trainer: NFRIModelTrainer) -> list[str]:
    ...
    try:
        ...
        lines.append("- (computed at retrain time on training sample; "
