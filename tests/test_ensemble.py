...
from nrfi.ensemble import (StackedEnsemble, n_eff_for_game,
                           purged_walk_forward_folds, shrink_to_venue)
...
def test_purged_folds_respect_gap_and_expansion():
    ...
    folds = list(purged_walk_forward_folds(dates, n_folds=4, purge_days=7,
                                           min_train=50))
    assert len(folds) >= 2
    prev_tr_max = -1
    for tr, va in folds:
...
def test_shrinkage_math_exact():
    ...
    assert shrink_to_venue(0.60, None, n_eff=0.0) == pytest.approx(0.60)   # no venue: untouched
