...
class RichBuilder(FeatureBuilder):
    ...
    def __init__(self):
        ...
        pg = pd.DataFrame({
            "pitcher_id": [1] * n + [2] * n, "game_date": list(dates) * 2,
            "earned_runs": 2, "runs_allowed": 2, "hits": 5, "walks": 2,
            "strikeouts": 6, "innings_pitched": 6.0, "opponent_team": "X",
        })
        pfi = pd.DataFrame({
            "pitcher_id": [1] * n + [2] * n, "game_date": list(dates) * 2,
            "first_inning_runs": [0, 1] * n, "first_inning_hits": 1,
            "first_inning_walks": 0,
        })
        sc = pd.DataFrame({
            "pitcher_id": [1] * n + [2] * n, "game_date": list(dates) * 2,
            "exit_velocity_sum": 880.0, "barrels": 1, "hard_hits": 4,
            "whiffs": 10, "swings": 40, "batted_balls": 10,
        })
        tg = pd.DataFrame({
            "team": ["NYY"] * n + ["BOS"] * n, "game_date": list(dates) * 2,
            "runs": 4, "hits": 9, "at_bats": 34, "total_bases": 15,
            "times_on_base": 12, "plate_appearances": 39,
            "woba_num": 11.0, "woba_den": 39.0,
        })
        tfi = pd.DataFrame({
            "team": ["NYY"] * n + ["BOS"] * n, "game_date": list(dates) * 2,
        ...
        bat = pd.DataFrame({
            ...
            "game_date": list(dates) * 6, "woba_num": 1.4, "woba_den": 4.2,
            "times_on_base": 2, "plate_appearances": 4,
        })
        parks = pd.DataFrame({"venue_id": [7], "runs_factor": [1.02],
                              "hr_factor": [1.0], "hits_factor": [1.0]})
        super().__init__(sf=None, raw_frames={
            "pitcher_games": pg, "pitcher_fi": pfi, "statcast_pitcher": sc,
            "team_games": tg, "team_fi": tfi, "batters": bat, "parks": parks})
...
def _game(**kw):
    g = {"game_id": "g1", "game_date": "2026-07-12",
         "home_team": "BOS", "away_team": "NYY",
         "home_pitcher_id": 2, "away_pitcher_id": 1,
         "home_pitcher_name": "H", "away_pitcher_name": "A",
         "venue_id": 7, "is_dome": True,
         "weather": None, "cf_azimuth_deg": None,
...
def _odds(age_sec=120, books=3, p_novig=(0.47, 0.48, 0.49)):
    ...
    return {("BOS", "NYY"): {
        "books": {f"book{i}": {"yrfi_prob_novig": p_novig[i % len(p_novig)],
                               "nrfi_american": -120 - i,
...
def test_low_coverage_blocks():
    empty = FeatureBuilder(sf=None, raw_frames={
        k: pd.DataFrame() for k in ("pitcher_games", "pitcher_fi",
                                    "statcast_pitcher", "team_games",
                                    "team_fi", "batters", "parks")})
    empty.prepare(max_date="2026-07-12")
    row = _predictor(empty).score_game(
        _game(is_dome=False, lineups=None), _odds(), NOW)
    assert row["status"] == "BLOCKED"
...
def test_data_health_dot():
    ...
    assert data_health([ok, {"status": "DEGRADED", "predicted_at": fresh}],
                       NOW) == "amber"
...
def test_display_contract_zero_edge_and_nulls():
    assert display_fields({"p_yrfi": 0.4, "p_yrfi_market": 0.4, "edge": 0.0}
                          )["edge_pct"] == 0.0        # 0.0 is a value
    blocked = display_fields({"p_yrfi": None, "p_yrfi_market": None,
                              "edge": None})
    assert blocked == {"nrfi_pct": None, "market_nrfi_pct": None,
                       "edge_pct": None}               # renders BLOCKED/UNAVAILABLE
    d = display_fields({"p_yrfi": 0.388, "p_yrfi_market": 0.416,
                        "edge": 0.388 - 0.416})
