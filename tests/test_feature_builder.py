...
def test_leakage_strictly_before_as_of():
    rows = _pitcher_rows()
    rows = {key: values + [extra] for (key, values), extra in zip(
        rows.items(),
    ...
    for name in without_same_day:
        ...
        assert (left == right) or (np.isnan(left) and np.isnan(right)), f"leak via {name}"


def test_missing_is_nan_and_flagged_never_default():
    features = _build(_frames(_pitcher_rows()), _game())
    assert features["home_p_missing"] == 1.0
    assert all(np.isnan(value) for name, value in features.items()
